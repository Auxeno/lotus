...
from .ddpg import DDPG, ActorState, CriticState, DDPGState
...
class TD3(DDPG):
    ...
    def learn(
        self, key: PRNGKey, agent_state: AgentState, batch: ArrayTree, train_actor: bool
    ) -> AgentState:
        ...
        agent_state = jax.lax.cond(
            train_actor, lambda x: actor_learn(x), lambda x: x, agent_state
        )
    ...
    @staticmethod
    def train(agent: "TD3", seed: int = 0) -> dict:
        """Main training loop."""
        ...
        def train_step(carry: dict, _: Any) -> tuple[dict, None]:
            ...
            train_actor = (
                global_step
                // (agent.rollout_steps * agent.num_envs)
                % agent.actor_delay
                == 0
            )
            agent_state = jax.lax.cond(
