...
from chex import ArrayTree, PRNGKey, Scalar
from flax.struct import dataclass, field


@dataclass
class BufferState:
    data: ArrayTree = field(
        pytree_node=True
    )  # pytree with leading dimensions (max_steps, num_envs)
...
class Buffer:
    @staticmethod
    def init_buffer(
        sample_item: ArrayTree, num_envs: Scalar, max_size: Scalar
    ) -> BufferState:
    ...
    def push(buffer_state: BufferState, item: ArrayTree) -> BufferState:
        ...
        new_idx = (buffer_state.idx + 1) % (
            buffer_state.max_size // buffer_state.num_envs
        )
        new_size = jnp.minimum(
            buffer_state.size + buffer_state.num_envs, buffer_state.max_size
        )
    ...
    def sample(
        key: PRNGKey, buffer_state: BufferState, batch_size: Scalar
    ) -> ArrayTree:
        ...
        step_indices = jax.random.randint(
            ...
            shape=(batch_size,),
            minval=0,
            maxval=buffer_state.size // buffer_state.num_envs,
        )
        ...
        sampled_data = jax.tree.map(
            lambda data: data[step_indices, env_indices], buffer_state.data
        )
